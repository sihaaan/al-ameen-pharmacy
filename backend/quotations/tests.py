...
class QuotationWorkflowTests(APITestCase):
    ...
    def test_batch_po_evidence_scan_marks_checked_quotes_without_repeating(self, mock_search):
        ...
        self.assertEqual(first.data["candidates_found"], 0)
        ...
        self.assertIsNotNone(quotation.po_evidence_last_scanned_at)
        self.assertEqual(quotation.po_evidence_last_scan_count, 0)
        self.assertIsNone(draft.po_evidence_last_scanned_at)
        scanned_quote = next(item for item in list_response.data if item["id"] == quotation.id)
        self.assertEqual(scanned_quote["po_evidence_count"], 0)
        self.assertIsNotNone(scanned_quote["po_evidence_last_scanned_at"])

    @patch("quotations.quote_po_intelligence.gmail_fetch_message_metadata")
    @patch("quotations.quote_po_intelligence.gmail_search_messages")
    def test_batch_po_evidence_scan_processes_quotes_in_chunks(self, mock_search, mock_metadata):
        first_quote = self.create_quote()
        self.create_valid_line(first_quote)
        self.client.post(reverse("quotation-finalize", args=[first_quote.id]))
        first_quote.status = Quotation.STATUS_SENT
        first_quote.sent_at = timezone.now()
        first_quote.save(update_fields=["status", "sent_at", "updated_at"])
        second_quote = self.create_quote()
        self.create_valid_line(second_quote)
        self.client.post(reverse("quotation-finalize", args=[second_quote.id]))
...
class ContractIntelligenceWorkflowTests(APITestCase):
            ...
            attachments=[
                {
                    ...
                    "lines": [
                        ...
                        {"requested_item_name": "$INO Fercdulnr*"},
    ...
    def test_contract_cleanup_action_cleans_existing_review_rows(self):
        ...
        logo = ContractIntelligenceItem.objects.create(
            run=run,
            source=source,
            original_item_name="[ALEC logo - vector file RGB]",
            suggested_item_name="[ALEC logo - vector file RGB]",
            normalized_item_name="alec logo vector file rgb",
            status=ContractIntelligenceItem.STATUS_SUGGESTED,
        )
        phone = ContractIntelligenceItem.objects.create(
            run=run,
            source=source,
            original_item_name="Mahammad Suleman 0505456388",
            suggested_item_name="Mahammad Suleman 0505456388",
    ...
    @patch("quotations.contract_intelligence.gmail_fetch_message_metadata")
    @patch("quotations.contract_intelligence.gmail_search_messages")
    def test_contract_full_discovery_runs_multiple_gmail_batches(self, mock_search, mock_metadata):
        GmailOAuthConnection.objects.create(
            user=self.staff,
            email="pharmacy@example.com",
            access_token_encrypted="token",
        ...
        response = self.client.post(
            reverse("quotation-contract-intelligence-run-discover-all", args=[run.id]),
            {"batch_size": 2, "max_batches": 5},
            format="json",
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["result"]["batches"], 2)
        self.assertEqual(response.data["result"]["created"], 3)
        self.assertEqual(ContractIntelligenceSource.objects.filter(run=run, status="candidate").count(), 3)
        self.assertEqual(mock_search.call_count, 2)
        self.assertEqual(mock_search.call_args_list[1].kwargs["page_token"], "NEXT_PAGE")

    def test_contract_intelligence_run_can_be_deleted_for_clean_restart(self):
        run = ContractIntelligenceRun.objects.create(
            company=self.company,
            target_company_name="ALEC",
            created_by=self.staff,
        ...
        response = self.client.delete(reverse("quotation-contract-intelligence-run-detail", args=[run.id]))

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(ContractIntelligenceRun.objects.filter(id=run.id).exists())
        self.assertFalse(ContractIntelligenceSource.objects.filter(id=source.id).exists())
        self.assertFalse(ContractIntelligenceItem.objects.filter(run_id=run.id).exists())
