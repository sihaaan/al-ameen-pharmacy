...
const draft = { id: 20, company: 1, customer_name: company.name, customer_address: company.billing_address, customer_trn: company.trn,
  ...
  invoice_number: '00604670/26', revision: 1, status: 'draft', notes: '', subtotal: '20.00', vat_total: '1.00', total: '21.00',
...
beforeEach(() => {
  ...
  quotationAPI.taxInvoices.issue.mockResolvedValue({ data: { ...draft, status: 'issued', revision: 2 } });
...
test('manual draft saves item fields and issuance requires review of the saved draft', async () => {
  ...
  await screen.findByText(draft.invoice_number);
  ...
  await waitFor(() => expect(screen.getByLabelText('Full customer name')).toHaveValue('Resort LLC'));
  fireEvent.change(screen.getByLabelText('Invoice number'), { target: { value: draft.invoice_number } });
  fireEvent.change(screen.getByLabelText('Full customer name'), { target: { value: 'Resort Hotel and Hospitality L.L.C' } });
  fireEvent.change(screen.getByLabelText('Full customer billing address'), { target: { value: 'Office 216\nDubai, UAE' } });
  ...
  await waitFor(() => expect(quotationAPI.taxInvoices.create).toHaveBeenCalledWith(expect.objectContaining({ company: '1', invoice_number: draft.invoice_number,
    customer_name: 'Resort Hotel and Hospitality L.L.C', customer_address: 'Office 216\nDubai, UAE', customer_trn: company.trn,
    lines: [expect.objectContaining({ item_name: 'Gauze', unit_price: '10', vat_rate: '5' })] })));
  const review = await screen.findByRole('checkbox', { name: /I checked the invoice number/ });
  expect(screen.getByRole('button', { name: 'Issue tax invoice' })).toBeDisabled();
  ...
  expect(screen.getByLabelText('Invoice number')).toHaveValue(draft.invoice_number);
...
test('quotation scan requires applying its preview and keeps unknown price and VAT blank', async () => {
  ...
  render(<TaxInvoiceManager />);
  fireEvent.click(screen.getByRole('button', { name: 'New tax invoice' }));
  fireEvent.change(screen.getByLabelText('Invoice number'), { target: { value: '604670' } });
  ...
  expect(screen.getByLabelText('Invoice number')).toHaveValue('604670');
...
test('opening scrolls to the invoice and editing resets review until the next save', async () => {
  ...
  fireEvent.click(await screen.findByRole('checkbox', { name: /I checked the invoice number/ }));
...
test('choosing a company loads its billing address from the detail endpoint', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'New tax invoice' }));
  fireEvent.change(screen.getByLabelText('Company'), { target: { value: '1' } });
  await waitFor(() => expect(screen.getByLabelText('Full customer billing address')).toHaveValue(company.billing_address));
  expect(quotationAPI.companies.retrieve).toHaveBeenCalledWith('1');
});

test('saved drafts with missing manual number or customer TRN cannot be issued', async () => {
  quotationAPI.taxInvoices.retrieve.mockResolvedValue({ data: { ...draft, invoice_number: null, customer_trn: '' } });
  render(<TaxInvoiceManager />);
  fireEvent.click(await screen.findByRole('button', { name: 'Open invoice' }));
  await screen.findByText(/Before issuing, complete: invoice number, 15-digit customer TRN/);
  expect(screen.getByRole('button', { name: 'Issue tax invoice' })).toBeDisabled();
  expect(screen.queryByRole('checkbox', { name: /I checked the invoice number/ })).not.toBeInTheDocument();
  fireEvent.change(screen.getByLabelText('Invoice number'), { target: { value: '604671' } });
